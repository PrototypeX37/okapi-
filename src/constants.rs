...
lazy_static! {
    ...
    pub static ref EXTERNAL_URL: String =
        var("EXTERNAL_URL").unwrap_or(format!("http://51.222.106.141:{}", *PORT));
    ...
    pub static ref RACES: HashMap<&'static str, RgbaImage> = {
		...
		all_races.insert(
            "shadeborn",
            load_from_memory(include_bytes!("../assets/images/out/casts/shadeborn.png"))
                .expect("Could not load image")
                .into_rgba8(),
        );
				all_races.insert(
            "null",
            load_from_memory(include_bytes!("../assets/images/out/casts/null.png"))
                .expect("Could not load image")
                .into_rgba8(),
        );
    ...
    pub static ref CLASSES: HashMap<&'static str, RgbaImage> = {
		...
		all_classes.insert(
            "paladin",
            load_from_memory(include_bytes!("../assets/images/out/casts/paladin.png"))
                .expect("Could not load image")
                .into_rgba8(),
        );
				all_classes.insert(
            "reaper",
            load_from_memory(include_bytes!("../assets/images/out/casts/reaper.png"))
                .expect("Could not load image")
                .into_rgba8(),
        );
